...
export class OrderHelpers {
  ...
  static getOrderAccountCodes(order: IOrder): IOrderTaxes[] {
    ...
    const itemsTaxes = (order.items || []).reduce((acc, item) => {
      ...
      switch (item.type) {
        case ItemType.PRODUCT: {
          ...
          if (!accountCodeId) console.error('No tax found for product', product.productVariation);

          if (!acc[accountCodeId]) {
            acc[accountCodeId] = 0;
          }

          acc[accountCodeId] += ProductHelpers.getPlainProductPrice(product, order.mode) || 0;
        ...
        case ItemType.PACK: {
          ...
          if (!accountCodeId) console.error('No tax found for pack', pack);

          if (!acc[accountCodeId]) {
            acc[accountCodeId] = 0;
          }

          acc[accountCodeId] += PackHelpers.getPlainPackPrice(pack, order.mode) || 0;
  ...
  static appendProductOptionAccountCodes(acc: any, options: IOrderOption[], mode: OrderMode) {
    return options?.reduce((products, option) => {
      products.push(...option.products.filter(product => product.quantity > 0))
      return products;
    }, [] as IOrderProduct[])?.forEach(product => {
      ...
      acc[accountCodeId] += ProductHelpers.getOptionProductPrice(product, mode) || 0;
